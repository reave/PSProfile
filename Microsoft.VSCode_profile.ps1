...
"@
    ...
    #- Oh-My-Posh
    $ohMyPoshPath = Get-Command oh-my-posh -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source
    if (-Not $ohMyPoshPath) {
        # Check common install locations for Oh-My-Posh
        $possiblePaths = @(
        ...
        if (-Not $ohMyPoshPath) {
            Write-Warning "Oh-My-Posh not found. Please install Oh-My-Posh to enable prompt theming."
            return
        }
        else {
            & $ohMyPoshPath init pwsh --config "$env:POSH_THEMES_PATH\M365Princess.omp.json" | Invoke-Expression
        }
    }
    Else {
        & $ohMyPoshPath init pwsh --config "$env:POSH_THEMES_PATH\M365Princess.omp.json" | Invoke-Expression
...
ElseIf ($IsMacOS) {
    ...
    $ohMyPoshPath = Get-Command oh-my-posh -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source
    if (-Not $ohMyPoshPath) {
        # Check common install locations for Oh-My-Posh on macOS
        $possiblePaths = @(
            "/usr/local/bin/oh-my-posh", # Manual install or Intel-based Macs using Homebrew
            "/opt/homebrew/bin/oh-my-posh", # Apple Silicon Macs using Homebrew
            "/usr/bin/oh-my-posh" # Manual install
        )
        foreach ($path in $possiblePaths) {
            if (Test-Path -Path $path) {
                $ohMyPoshPath = $path
