Function Test-DHCPPxe {
    <#
    .SYNOPSIS
        Test and discover PXE
    .DESCRIPTION
        Test and discover PXE on the current network
    .PARAMETER MacAddressString
        Define a mac address to use for the request

        Default is "AA:BB:CC:DD:EE:FF"
    .PARAMETER UUIDString
        Define a UUID to use for the request

        Default is "AABBCCDD-AABB-AABB-AABB-AABBCCDDEEFF"
    .PARAMETER Option60String
        Defines a DHCP Option 60 string to pass as part of the request

        Default is PXEClient
    .PARAMETER ProcessorArchitecture
        Determiens the process architecture we present during the PXEDHCP Request
    .PARAMETER DiscoverTimeout
        Length of time (in seconds) to spend waiting for offers if the
        connection does not timeout first.

        Default is 60 seconds
    .EXAMPLE
        PS C:\> Test-DHCPPxe
        Will broadcast a DHCP Discover Request and PXE Option Request and output the response we receive
    .NOTES
        ...
        Major Rework Author: Andreas Hammarskjöld @ 2Pint Software http://2pintsoftware.com
    ...
    [void]$UdpSocket.SendTo($Message, $EndPoint)
